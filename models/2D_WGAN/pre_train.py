...
def training_pre(model, dataloader, dataset_sizes,
                 criterion, device, path, gpu=False,
                 max_step=500000, lr=1e-4, 
                 batch_size=2, cube_size=64, 
...
#                                 print('Step: {}, loss= {:.4f}'.format(step, loss.item()))
                        if (step % int(max_step // 10)) ==0:
                            ...
                            torch.save(model,path + 'pretrained_G_step{}'.format(step))
                        ...
                        if (step == max_step):
                            ...
                            #else:
                            torch.save(model.state_dict(),path + 'pretrained_G_step{}'.format(step))
                            return model
                # statistics
